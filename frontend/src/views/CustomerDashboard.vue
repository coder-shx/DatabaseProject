...
export default {
  ...
  methods: {
    ...
    async urgeOrder(order) {
      try {
        console.log('催促订单:', order.id);
        const response = await this.$axios.put(`/repair-orders/${order.id}/urge`);
        
        // 更新本地订单状态
        const orderIndex = this.repairOrders.findIndex(o => o.id === order.id);
        if (orderIndex !== -1) {
    ...
    isOrderUrged(order) {
      // 如果最后催单时间在30分钟内，则不允许再次催单
      if (!order.lastUrgedAt) return false;
      const thirtyMinutesAgo = new Date(Date.now() - 30 * 60 * 1000);
      return new Date(order.lastUrgedAt) > thirtyMinutesAgo;
    }
  }
}
</script>
