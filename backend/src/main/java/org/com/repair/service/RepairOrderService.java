...
public class RepairOrderService {
    ...
    public RepairOrderResponse updateRepairOrderStatus(Long orderId, RepairOrder.RepairStatus newStatus, Double materialCost) {
        System.out.println("开始更新订单状态 - 订单ID: " + orderId + ", 新状态: " + newStatus);
        
        RepairOrder repairOrder = repairOrderRepository.findById(orderId)
                .orElseThrow(() -> new RuntimeException("维修订单不存在"));
        
        RepairOrder.RepairStatus oldStatus = repairOrder.getStatus();
        System.out.println("订单原状态: " + oldStatus + " -> 新状态: " + newStatus);
        ...
        if (oldStatus == RepairOrder.RepairStatus.ASSIGNED && newStatus == RepairOrder.RepairStatus.IN_PROGRESS) {
            ...
            repairOrder.setStartedAt(startTime);
            System.out.println("设置开始时间: " + startTime);
        }
        ...
        // 强制保存并刷新
        RepairOrder savedOrder = repairOrderRepository.saveAndFlush(repairOrder);
        System.out.println("订单状态已保存 - ID: " + savedOrder.getId() + ", 状态: " + savedOrder.getStatus() + 
                           ", 开始时间: " + savedOrder.getStartedAt());
    ...
    @Transactional
    public RepairOrderResponse urgeOrder(Long orderId) {
        RepairOrder order = repairOrderRepository.findById(orderId)
                .orElseThrow(() -> new RuntimeException("Order not found"));

        // Check if order can be urged
        if (order.getStatus() != RepairStatus.ASSIGNED && order.getStatus() != RepairStatus.IN_PROGRESS) {
            throw new RuntimeException("Order cannot be urged in current status: " + order.getStatus());
        }

        // Update urge information
        order.setLastUrgedAt(new Date());
    ...
    @Transactional
    public RepairOrderResponse rejectOrder(Long orderId) {
        RepairOrder order = repairOrderRepository.findById(orderId)
                .orElseThrow(() -> new RuntimeException("Order not found"));

        // Check if order is in a state that can be rejected
        if (order.getStatus() != RepairStatus.ASSIGNED && order.getStatus() != RepairStatus.IN_PROGRESS) {
            throw new RuntimeException("Order cannot be rejected in current status: " + order.getStatus());
        }

        // Update order status to REJECTED
        order.setStatus(RepairStatus.REJECTED);
        order.setUpdatedAt(new Date());

        // Clear current technician assignments
        order.getTechnicians().clear();

        // Auto-assign to new technicians
        Set<Technician> newTechnicians = autoAssignmentService.assignTechnicians(order);
        if (!newTechnicians.isEmpty()) {
            order.getTechnicians().addAll(newTechnicians);
