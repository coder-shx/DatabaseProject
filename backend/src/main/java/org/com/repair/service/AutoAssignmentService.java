...
public class AutoAssignmentService {
    ...
    /**
     * 自动分配单个技师到维修工单（基于指定的技师工种类型，并排除已拒绝的技师）
     * @param requiredSkillType 需要的技能类型
     * @param order 维修工单，用于检查已拒绝的技师
     * @return 分配的技师
     */
    public Technician autoAssignBestTechnician(SkillType requiredSkillType, RepairOrder order) {
        ...
        // 移除已经拒绝过的技师
        if (order != null && order.getRejectedTechnicians() != null && !order.getRejectedTechnicians().isEmpty()) {
            availableTechnicians.removeAll(order.getRejectedTechnicians());
            
            // 如果移除已拒绝的技师后没有可用技师，返回null
            if (availableTechnicians.isEmpty()) {
                return null;
            }
        }
    ...
    public Technician autoAssignBestTechnician(SkillType requiredSkillType) {
        return autoAssignBestTechnician(requiredSkillType, null);
    }
    ...
    public Set<Technician> autoAssignTechnicians(RepairOrder repairOrder, Set<SkillType> requiredSkillTypes) {
        ...
        for (SkillType skillType : requiredSkillTypes) {
            Technician bestTechnician = autoAssignBestTechnician(skillType, repairOrder);
    ...
    /**
     * Assigns technicians to a repair order based on required skills
     * @param order The repair order that needs technicians
     * @return Set of assigned technicians
     */
    public Set<Technician> assignTechnicians(RepairOrder order) {
        Set<Technician> assignedTechnicians = new HashSet<>();
        
        // If order has a specific required skill type
        if (order.getRequiredSkillType() != null) {
            Technician bestTechnician = autoAssignBestTechnician(order.getRequiredSkillType(), order);
            if (bestTechnician != null) {
                assignedTechnicians.add(bestTechnician);
            }
            return assignedTechnicians;
        ...
        // If no specific skill type is set, determine required skills from description
        Set<SkillType> requiredSkills = determineRequiredSkillTypes(order.getDescription());
        
        // Assign best technician for each required skill type
        for (SkillType skillType : requiredSkills) {
            Technician bestTechnician = autoAssignBestTechnician(skillType, order);
            if (bestTechnician != null) {
                assignedTechnicians.add(bestTechnician);
            }
        }
    ...
    public void reassignOrder(RepairOrder order) {
        // 清空当前技师列表（拒绝的技师已经在调用方法中添加到拒绝列表）
        if (order.getTechnicians() != null) {
            order.getTechnicians().clear();
        }

        // 寻找新的技师，排除已拒绝的技师
        Technician newTechnician = autoAssignBestTechnician(order.getRequiredSkillType(), order);
        if (newTechnician == null) {
            throw new IllegalStateException("没有合适的技师可以重新分配（已排除先前拒绝的技师）");
        }
        
        if (order.getTechnicians() == null) {
            order.setTechnicians(new HashSet<>());
        }
